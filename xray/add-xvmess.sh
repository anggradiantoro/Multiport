...
grpc="$(cat ~/log-install.txt | grep -w "XRAY VMESS GRPC" | cut -d: -f2|sed 's/ //g')"
...
},{"id": "'""$uuid""'","alterId": '"0"',"email": "'""$user""'"' /etc/xray/v2ray-grpc.json
...
EOF
cat>/etc/xray/v2ray-$user-grpc.json<<EOF
{
      "v": "2",
      "ps": "${user}",
      "add": "${domain}",
...
xrayv2ray2="vmess://$(base64 -w 0 /etc/xray/v2ray-$user-grpc.json)"
